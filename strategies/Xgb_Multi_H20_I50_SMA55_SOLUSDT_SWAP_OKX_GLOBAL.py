...
def wma(df: pd.DataFrame, window: int) -> np.ndarray:
    """计算加权移动平均线指标"""
    return ta.WMA(df["close"].to_numpy(), window)


def ht_trendline(df: pd.DataFrame) -> np.ndarray:
    """计算HT_TRENDLINE指标"""
    return ta.HT_TRENDLINE(df["close"].to_numpy())
...
class Xgb_Multi_H20_I50_SMA55_SOLUSDT_SWAP_OKX_GLOBAL(TargetPosTemplate):
    """
    Based on XGBoost Algorithm
    Features: ['HT_TRENDLINE', 'NATR_10', 'SMA_10', 'WMA_45', 'SMA_55']
    """
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.bg = BarGenerator(self.on_bar, 50, self.on_window_bar)
        
        # Ensure ArrayManager is large enough for the max window needed
        self.am = ArrayManager(size=300 + 30)

        # Load Model - 支持多种路径解析
        self.model_path = "strategies/Xgb_Multi_H20_I50_SMA55_SOLUSDT_SWAP_OKX_GLOBAL.joblib"
        self.model = None
        load_errors = []
    ...
    def on_window_bar(self, bar: BarData):
        ...
        val_HT_TRENDLINE = ht_trendline(df)
        features.append(val_HT_TRENDLINE[-1] if isinstance(val_HT_TRENDLINE, np.ndarray) else val_HT_TRENDLINE)
        val_NATR_10 = natr(df, 10)
        features.append(val_NATR_10[-1] if isinstance(val_NATR_10, np.ndarray) else val_NATR_10)
        val_SMA_10 = sma(df, 10)
        features.append(val_SMA_10[-1] if isinstance(val_SMA_10, np.ndarray) else val_SMA_10)
        val_WMA_45 = wma(df, 45)
        features.append(val_WMA_45[-1] if isinstance(val_WMA_45, np.ndarray) else val_WMA_45)
        val_SMA_55 = sma(df, 55)
        features.append(val_SMA_55[-1] if isinstance(val_SMA_55, np.ndarray) else val_SMA_55)
