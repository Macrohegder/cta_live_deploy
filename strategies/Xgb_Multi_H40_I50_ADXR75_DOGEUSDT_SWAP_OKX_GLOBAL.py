...
def adxr(df: pd.DataFrame, window: int) -> np.ndarray:
    """计算ADXR指标"""
    return ta.ADXR(df["high"].to_numpy(), df["low"].to_numpy(), df["close"].to_numpy(), window)



class Xgb_Multi_H40_I50_ADXR75_DOGEUSDT_SWAP_OKX_GLOBAL(TargetPosTemplate):
    """
    Based on XGBoost Algorithm
    Features: ['NATR_55', 'WMA_5', 'ADXR_75']
    """
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        # Load Model - 支持多种路径解析
        self.model_path = "strategies/Xgb_Multi_H40_I50_ADXR75_DOGEUSDT_SWAP_OKX_GLOBAL.joblib"
        self.model = None
        load_errors = []
    ...
    def on_window_bar(self, bar: BarData):
        ...
        val_WMA_5 = wma(df, 5)
        features.append(val_WMA_5[-1] if isinstance(val_WMA_5, np.ndarray) else val_WMA_5)
        val_ADXR_75 = adxr(df, 75)
        features.append(val_ADXR_75[-1] if isinstance(val_ADXR_75, np.ndarray) else val_ADXR_75)
