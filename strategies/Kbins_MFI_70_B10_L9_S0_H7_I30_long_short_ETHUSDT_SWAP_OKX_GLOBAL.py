...
def mfi(df: pd.DataFrame, window: int) -> np.ndarray:
    """计算MFI指标"""
    return ta.MFI(df["high"].to_numpy(), df["low"].to_numpy(), df["close"].to_numpy(), df['volume'].to_numpy(), window)

class Kbins_MFI_70_7_I30_long_short(TargetPosTemplate):
    """
    基于MFI_70特征和7根bar持仓周期的KBinsovernight策略
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        if len(self.windows) != 0:
            # 确保ArrayManager足够大
            size = max(self.windows) + 20
        self.am = ArrayManager(size=size)

        # 加载预训练模型和特征计算函数
        with open("strategies/Kbins_MFI_70_B10_L9_S0_H7_I30_long_short_ETHUSDT_SWAP_OKX_GLOBAL.model", "rb") as f:
            self.est = pickle.load(f)
        self.feature_func = mfi
