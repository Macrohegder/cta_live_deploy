...
def mom(df: pd.DataFrame, window: int) -> np.ndarray:
    """计算MOM指标"""
    return ta.MOM(df["close"].to_numpy(), window)


def minus_dm(df: pd.DataFrame, window: int) -> np.ndarray:
    """计算MINUS_DM指标"""
    return ta.MINUS_DM(df["high"].to_numpy(), df["low"].to_numpy(), window)



class Xgb_Multi_H60_I60_MINUSDM55_DOGEUSDT_SWAP_OKX_GLOBAL(TargetPosTemplate):
    """
    Based on XGBoost Algorithm
    Features: ['MOM_80', 'EMA_65', 'MINUS_DM_55']
    """
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        # Load Model - 支持多种路径解析
        self.model_path = "strategies/Xgb_Multi_H60_I60_MINUSDM55_DOGEUSDT_SWAP_OKX_GLOBAL.joblib"
        self.model = None
        load_errors = []
    ...
    def on_window_bar(self, bar: BarData):
        ...
        val_MOM_80 = mom(df, 80)
        features.append(val_MOM_80[-1] if isinstance(val_MOM_80, np.ndarray) else val_MOM_80)
        val_EMA_65 = ema(df, 65)
        features.append(val_EMA_65[-1] if isinstance(val_EMA_65, np.ndarray) else val_EMA_65)
        val_MINUS_DM_55 = minus_dm(df, 55)
        features.append(val_MINUS_DM_55[-1] if isinstance(val_MINUS_DM_55, np.ndarray) else val_MINUS_DM_55)
